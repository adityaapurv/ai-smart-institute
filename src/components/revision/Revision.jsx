...
const Revision = () => {
  ...
  const handleDownloadPDF = () => {
    ...
    pdf.setFontSize(16);
    pdf.text(
      `${paperType.toUpperCase()} - Difficulty: ${difficulty.charAt(0).toUpperCase() + difficulty.slice(1)}${subject ? ` - Subject: ${subject}` : ''}${topic ? ` - Topic: ${topic}` : ''}`,
      margin,
      y
    );
    y += 30;
  ...
  return (
    <Container>
      ...
      <Form onSubmit={handleSubmit}>
        ...
        <Field>
          <Label htmlFor="subject">Particular Subject (optional):</Label>
          <Input
            id="subject"
            type="text"
            value={subject}
            onChange={(e) => setSubject(e.target.value)}
            placeholder="Enter subject if any"
          />
        </Field>
        <Field>
